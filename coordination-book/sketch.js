...
/* ✅ 외부 JSON fetch 없이, 코드 내부 SEED로 초기화 */

const INITIAL_SEED =
/* ==== 여기에 너의 백업 JSON을 통째로 붙여넣어 ==== */
{
  /* 예시)
  "2025-11-04": {
    "notes": "오늘은 발표날, 편한 발표룩!",
    "moods": ["😌 차분","⚡ 집중"],
    "photo": "data:image/jpeg;base64,...."  // 또는 images/xxx.jpg (그 파일도 레포에 있어야 함)
  }
  */
};
/* ===== 붙여넣기 끝 ===== */
...
function App(){
  ...
  const [book, setBook] = useLS(LS_KEY, {});
  const [openDay, setOpenDay] = React.useState(null);

  const cells = React.useMemo(()=>monthGrid(year, month), [year, month]);

  // ✅ 처음 방문(또는 비어 있음)에는 코드 내장 SEED로 초기화 (fetch 안 함)
  React.useEffect(()=>{
    try {
      const saved = localStorage.getItem(LS_KEY);
      const hasData = saved && saved !== "{}";
      if (!hasData) {
        const seed = (INITIAL_SEED && typeof INITIAL_SEED === 'object') ? INITIAL_SEED : {};
        setBook(seed);
      ...
      console.warn("초기화 중 오류: SEED로 재설정", e);
      const seed = (INITIAL_SEED && typeof INITIAL_SEED === 'object') ? INITIAL_SEED : {};
      setBook(seed);
      localStorage.setItem(LS_KEY, JSON.stringify(seed));
    }
  ...
  const doBackup = () => {
    ...
    const now = new Date();
    a.href = url;
    a.download = `coordination_backup_${now.getFullYear()}-${pad(now.getMonth()+1)}-${pad(now.getDate())}.json`;
    a.click();
  ...
  const resetToSeed = () => {
    if (!confirm("초기 SEED로 되돌릴까요? (현재 기록은 덮어써집니다)")) return;
    const seed = (INITIAL_SEED && typeof INITIAL_SEED === 'object') ? INITIAL_SEED : {};
    setBook(seed);
    localStorage.setItem(LS_KEY, JSON.stringify(seed));
    alert("초기 상태로 리셋 완료!");
    ...
    <div className="max-w-5xl mx-auto">
      <header className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* (선택) 유틸 */}
          <div className="flex items-center gap-2 rounded-2xl bg-white p-2 shadow">
            <button className="px-3 py-2 rounded-xl border hover:bg-stone-50" onClick={doBackup}>💾 백업</button>
            ...
            <button className="px-3 py-2 rounded-xl border hover:bg-stone-50" onClick={resetToSeed}>↺ 초기화</button>
          </div>
        </div>
      </header>
